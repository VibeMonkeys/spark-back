...
class AchievementDomainService {
    ...
    private fun checkCategoryAchievements(category: com.monkeys.spark.domain.vo.mission.MissionCategory): List<AchievementType> {
        val achievements = mutableListOf<AchievementType>()
        
        // 실제로는 카테고리별 완료 횟수를 확인해야 하지만 
        // 여기서는 단순화하여 구현
        when (category) {
            com.monkeys.spark.domain.vo.mission.MissionCategory.HEALTH -> {
                achievements.add(AchievementType.HEALTH_SPECIALIST)
            }
            com.monkeys.spark.domain.vo.mission.MissionCategory.CREATIVE -> {
                achievements.add(AchievementType.CREATIVE_ARTIST)
            }
            com.monkeys.spark.domain.vo.mission.MissionCategory.SOCIAL -> {
